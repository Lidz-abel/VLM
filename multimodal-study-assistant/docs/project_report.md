...
1. 使用 Gradio 构建演示界面。
2. 使用 PyMuPDF 将 PDF 渲染为页面 PNG。
3. 使用 Qwen2.5-VL-7B-Instruct 完成图片理解与页面解析。
4. 使用 BAAI/bge-small-zh-v1.5 生成页面文本向量。
5. 使用 FAISS IndexFlatIP 进行归一化向量检索。
6. 将检索到的页面内容作为依据生成面向学习场景的回答或笔记。
